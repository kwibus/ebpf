...
readonly rkt_dir="${SEMAPHORE_CACHE_DIR}/rkt-${rkt_version}"
readonly rkt="${rkt_dir}/rkt"
...
if [[ ! -f "${rkt}" ]]; then
  curl -LsS "https://github.com/coreos/rkt/releases/download/v${rkt_version}/rkt-v${rkt_version}.tar.gz" \
    -o "${SEMAPHORE_CACHE_DIR}/rkt-${rkt_version}.tgz"

  mkdir -p "${rkt_dir}"
  tar -xf "${SEMAPHORE_CACHE_DIR}/rkt-${rkt_version}.tgz" -C "${rkt_dir}" --strip-components=1
fi
...
sudo "${rkt}" image fetch --insecure-options=image "coreos.com/rkt/stage1-kvm:${rkt_version}" >/dev/null
...
for kernel_version in "${kernel_versions[@]}"; do
  ...
  sudo timeout --foreground --kill-after=10 2m \
    "${rkt}" \
    run --interactive \
    ...
    --mount=volume=src,target="/src/${SEMAPHORE_PROJECT_NAME}" \
    docker://golang:1.10-alpine \
    --exec=/bin/sh -- -c \
    "mount -t tmpfs tmpfs /tmp &&
      mount -t bpf bpf /sys/fs/bpf &&
      mount -t debugfs debugfs /sys/kernel/debug/ &&
      cd /src/${SEMAPHORE_PROJECT_NAME} &&
      go build -v ./... &&
      go test -coverprofile=coverage.txt -covermode=atomic -v ./..."

  # Determine exit code from pod status due to rkt#2777
  # https://github.com/coreos/rkt/issues/2777
  test_status=$(sudo "${rkt}" status "$(<rkt-uuid)" | awk '/app-/{split($0,a,"=")} END{print a[2]}')
